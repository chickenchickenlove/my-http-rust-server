...
use crate::http2::http2_stream::{new_stream_span, ActiveStreamCountCommand, DecodingHeaderState, Http2ChannelMessage, Http2Stream, Http2StreamHandler, ReaderChannelMessage};
use crate::http2::http2_stream_handler::Http2StreamHandler2;
...
impl Http2Connection2 {
    ...
    pub fn decode_headers(&mut self, decoder: &mut Http2HeaderDecoder, stream_id: u32, payload: Bytes) -> anyhow::Result<(HashMap<String, String>, HashSet<String>)>{
                        ...
                        ReaderChannelMessage::MaybeFrameParsed { frame: parsed_frame } => {
                            ...
                            let result = match parsed_frame {
                                ...
                                _ => {
                                    ...
                                    self.handle_stream_level_frame2(parsed_frame, &mut handler, &mut writer, init_window_size).await;

                                    let mut stream_handler = Http2StreamHandler2::new();

                                    if let Some(stream_ref) = self.conn_ctx.find_stream_mut_ref_by_stream_id(stream_id) {
                                        let stream_ctx = stream_ref.context_as_mut_ref();

                                        if stream_handler.should_composite_headers(stream_ref) {
                                            let Ok(header_bytes) = stream_handler.get_headers_bytes(stream_ref);
                                            // TODO: Send Message
                                            match self.decode_headers(&mut decoder)

                                            self.decode_headers(&mut decoder, stream_ctx.stream_id(), header_bytes);
                                            stream_handler.update_headers(stream_ref, )
                                        }

                                        if stream_handler.should_composite_trailers(stream_ref) {
                                            let Ok((trailer_bytestrailers, trailers)) = stream_handler.composite_trailers(stream_ref);
                                            // TODO: Send Message.
                                        }

                                        if stream_handler.should_dispatch(stream_ref) {
                                            let Ok(req_ctx) = stream_handler.dispatch_if_needed(stream_ref);
                                            // TODO: Spawn Tokio dispatcher.
    ...
    pub async fn handle_connection_level_frame2(&mut self,
                                               mut frame: FrameFacade,
                                               handler: &mut Http2Handler,
                                               writer: &mut BufWriter<OwnedWriteHalf>,
    ) -> anyhow::Result<()>{
    ...
    pub async fn handle_stream_level_frame2(&mut self,
                                           mut frame: FrameFacade,
                                           handler: &mut Http2Handler,
                                           writer: &mut BufWriter<OwnedWriteHalf>,
                                           init_window_size: u32
    ) -> anyhow::Result<()>
    {
        ...
        if Http2StreamHandler::should_init_stream(&frame, &self.conn_ctx) {
            ...
            //        It can not ensure that keep the this rules.
            match frame {
                HeadersFrame(_) => self.conn_ctx.update_max_active_stream_id(stream_id),
                _ => ()
            }

            if stream_id < self.conn_ctx.get_max_active_stream_id() {
                self.conn_ctx.update_error(Http2Error::ConnectionError(ErrorCode::ProtocolError).into());
                return Ok(())
            }

            let stream_span = new_stream_span(stream_id, init_window_size);
            let stream = Stream::init_stream(stream_id, init_window_size as u64);

            self.conn_ctx.add_stream2(stream_id, stream, stream_span.clone());
            self.conn_ctx.update_seen_max_stream_id(stream_id);
        };
        ...
        let handler = Http2StreamHandler2::new();
        if let Some(s) = self.conn_ctx.find_stream_by_stream_id(stream_id) {
            if let Ok(new_stream) = handler.handle_frame(s, frame) {
                self.conn_ctx.add_stream3(stream_id, new_stream);
            }
        }
